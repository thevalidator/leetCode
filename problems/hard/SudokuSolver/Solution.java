                    ...
                    if (board[i][j] == '.') {
                        ...
                        if (suitableCharacters.size() == 1) {
                            ...
                            for (char c : suitableCharacters) {
                                ...
                                for (int k = 0; k < 3; k++) {
                                    int hIndex = startIndex + k;
                                    int vIndex = startIndex + k;
                                    //System.out.println("field: (" + i + ":" + j + ") box: " + index + " h index: " + hIndex + " v index: " + vIndex);
                                    
                                    if ((hIndex == i && horisontals.get(hIndex).contains(c)) || 
                                            (hIndex == j && verticals.get(hIndex).contains(c))) {
                                        isSuits = false;
                                    } else if (!verticals.get(hIndex).contains(c) && !horisontals.get(vIndex).contains(c)) {
                                        isSuits = false;
                                    }
...
//                                            + (vIndex));
                                ...
                                if (isSuits && !squares.get(index).contains(c)) {
                                    board[i][j] = c;
                                    squares.get(index).add(c);
                                    verticals.get(j).add(c);
                                    horisontals.get(i).add(c);
